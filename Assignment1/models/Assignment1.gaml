...
global {
	...
	int neighbours_distance <- 10 min: 1 parameter: "Distance of perception:" category: "Population" max: 1000;
	//Shapefile to load
	file shape_file_name <- file("../gis/cambridge.shp") parameter: "Shapefile to load:" category: "GIS specific";
	//Shape of the environment
	geometry shape <- envelope(shape_file_name);
	//Square meters per people in m2
	int square_meters_per_people <- 1000 parameter: "Occupancy of people (in m2):" category: "GIS specific";
	...
	reflex save_result when: (time < 15){
		save ("cycle: "+ cycle + "; nbPeople: " + number_of_people) 
	   		to: "results.txt" type: "text" rewrite: (cycle = 0) ? true : false;
	}
	
	
	reflex stop_simulation when: (time = 15) {
    do pause ;
    }
